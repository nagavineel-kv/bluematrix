...
const TaskColumn= ({status, tasks, moveTask, setIsModalNewTaskOpen}: TaskColumnProps) => {
    ...
    return <div ref={(instance)=>{drop(instance)}} className={`sl:py-4 rounded-lg py-2 xl:px-2 ${isOver ? "bg-blue-100 dark:bg-neutral-950":""}`}>
        <div className='mb-3 flex w-full'>
            ...
            <div className='flex w-full items-center justify-between rounded-e-lg bg-white px-5 py-4 dark:bg-dark-secondary'>
                <h3 className='flex items-center text-lg font-semibold dark:text-white'>
                    {status} {" "}
                    <span className='ml-2 inline-block rounded-full bg-gray-200 p-1 text-center text-sm leading-none dark:bg-dark-tertiary' style={{width: "1.5rem", height:"1.5rem"}}>{tasksCount}</span>
                </h3>
...
const Task = ({task}: TaskProps) => {
    ...
    const formattedStartDate = task.startDate ? format(new Date(task.startDate), "PPP") : "";
        ...
        <div ref={(instance) => {drag(instance)}} className={`mb-4 rounded-md bg-white shadow dark:bg-dark-secondary ${isDragging ? "opacity-50" : "opacity-100"}`}>
            ...
            <div className='p-4 md:p-6 '>
                ...
                <div className='text-xs text-gray-500 dark:text-neutral-500'>
                    ...
                    {formattedStartDate && (
                        <div className="mb-2">
                            <span className="font-semibold text-green-700 dark:text-green-400">Start Date:</span>{" "}
                            <span>{formattedStartDate}</span>
                        </div>
                    )}
                    {formattedDueDate && (
                        <div className="mb-2">
                            <span className="font-semibold text-red-700 dark:text-red-400">Due Date:</span>{" "}
                ...
                <div className="mt-3 flex items-center justify-between">
                    <div className="flex -space-x-2 overflow-hidden">
                        {task.assignee && (
                            <Image key={task.assignee.userId} src={`/${task.assignee.profilePictureUrl}`} alt={task.assignee.username} title={`Assignee: ${task.assignee.username}`} width={30} height={30} className="h-8 w-8 rounded-full border-2 border-red-300 object-cover dark:border-red-400" />
                        )}
                        {task.author && (
                            <Image key={task.author.userId} src={`/${task.author.profilePictureUrl}`} alt={task.author.username} title={`Author: ${task.author.username}`} width={30} height={30} className="h-8 w-8 rounded-full border-2 border-green-300 object-cover dark:border-green-400" />
                        )}
                    </div>
